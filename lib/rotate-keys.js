...
const rotateKeys = async function rotateKeys(userName, options = { newKeyHandler: async () => {} }) {
  try {
    ...
    try {
      ...
      await options.newKeyHandler(userName, { accessKeyId, secretAccessKey });
    } catch (e) {
...
if (require.main === module) {
  (async () => {
    ...
    await rotateKeys(argv.username, {
      newKeyHandler: async (userName, { accessKeyId, secretAccessKey }) => {
        console.log(`New active key generated for ${userName}: ${accessKeyId}, ${secretAccessKey}.`);
      }
    });
  })();
}
